...
The state directory defaults to `~/.quotagauge`. On a machine that ran this project under its old name, the first command that resolves the default directory migrates a pre-existing `~/.headroom` there automatically (a directory rename, not a copy) and then never looks at the old path again. If the rename cannot complete for any reason, the old directory is left exactly as it was and used in place, so existing history is never lost or orphaned. Set `QUOTAGAUGE_STATE_DIR` to bypass this default (and its migration) entirely.
