...
That trust decision answers only "is this fit internally consistent," which is a different question from "is anything current still confirming it." A source that stops reporting mid-window can leave behind a perfectly steady, low-deviation historical trend that clears every one of those four thresholds even though nothing has confirmed it in hours. `status` and `hook` apply a second, independent check for exactly that reason, made of two parts: the current reading for the projection's exact source and window must be `fresh` (the same freshness state described above), AND usage must have demonstrably changed recently, not merely been re-captured recently. That second part exists because a session that climbs and then genuinely stalls can keep producing fresh-looking captures that all repeat the same plateaued percentage forever; the four trust-bar measurements are built from intervals that drop a trailing flat run entirely; they never see it, so they stay clean while the trend has visibly stopped. A projection reports when usage last actually moved as a plain fact (`latest_change_at`), and the policy compares that timestamp against a tolerance built from the projection's OWN fitted rate (how long its own pace predicts between whole-point changes), not a fixed window -- a fixed window would wrongly call a genuinely slow trend stale for most of its own cycle. Failing either half of this check leaves the projection silent regardless of how clean its own fit looks.
