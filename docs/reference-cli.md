...
`headroom status` and `headroom hook` additionally apply a burn-rate policy: a projection is shown to a human or a model only when its own measurements clear a conservative trust bar (low deviation between intervals, no single interval dominating the evidence, enough independent intervals, low drift between the window's early and late portions) AND there is currently fresh evidence backing the trend. "Currently fresh evidence" is two separate conditions, because they catch different failures: a fresh reading for that exact source and window (a recent capture happened at all), and `now` falling within a tolerance of `latest_change_at` (usage last demonstrably changed recently, not just that a capture recently repeated an old value). The second half matters because a session that climbs steadily and then genuinely stalls can keep producing fresh-looking captures indefinitely, all reporting the same plateaued percentage; the trust-bar measurements are computed from folded intervals that drop a trailing flat run's elapsed time entirely, so they stay clean and low-deviation even while the trend has visibly stopped, and only `latest_change_at` -- checked against the current time, not just the capture time -- catches that case. That tolerance is a multiple of the projection's OWN implied inter-change interval (1 / rate), not a fixed per-source window: a genuinely slow trend (for example Codex weekly usage climbing one point every two hours) has its own long, ordinary gap between whole-point changes, and comparing that against a short fixed window would call a perfectly healthy slow trend stale for most of every tick cycle. The trust thresholds were calibrated against this project's own capture history rather than picked from first principles alone; `max_raw_rate_ratio` (the raw, unfolded burst measurement) is deliberately excluded from the trust bar even though `json` and `doctor` still report it, because headroom's own capture cadence (often sub-minute, against usage quantized to whole percentage points) saturates that measurement on essentially any real segment, independent of whether the underlying usage is actually bursty -- it is structurally uninformative for input shaped like headroom's own. This policy exists precisely because a hard cutoff cannot live inside the projection itself; see [Why the bounds are sound](explanation-bounds.md) for why that separation matters and what the policy declines to claim. `status` prints one line per window whose projection clears every check; a declined, untrustworthy, or stale/stalled-evidence projection is silent there, matching `hook`'s existing behavior of saying nothing when there is nothing worth saying. On the `hook` path specifically, a history file larger than `HOOK_MAX_HISTORY_BYTES` (20 MB) skips burn-rate projections entirely rather than risk the hook's own timeout on an unbounded read; `status`, `json`, and `doctor` are not time-bound the same way and read the full file regardless of size.
