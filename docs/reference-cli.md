...
| `HEADROOM_UPDATE_CHECK` | Off | The exact value `1` allows `status` and `doctor` to query PyPI. An applicable cache for the installed version suppresses another query until that cache is 24 hours old. This is not a rate limit: changing the installed version, or a cache that is missing, replaced, unwritable, or otherwise inapplicable, makes another check due. Unset it to disable checking and show the local discovery hint in `status`. Set it to `0` to disable checking and suppress the hint. Every other value also disables checking. |
