...
`headroom status` and `headroom hook` additionally apply a burn-rate policy: a projection is shown to a human or a model only when its own measurements clear a conservative trust bar (low deviation between intervals, no single interval dominating the evidence, enough independent intervals, low drift between the window's early and late portions) AND there is a currently fresh reading for that exact source and window. The second condition matters on its own: an internally consistent historical fit built from a source that stopped reporting is not evidence of a live trend, only a historical one, so a stale or missing current reading suppresses the projection even when its own measurements are clean. The trust thresholds were calibrated against this project's own capture history rather than picked from first principles alone; `max_raw_rate_ratio` (the raw, unfolded burst measurement) is deliberately excluded from the trust bar even though `json` and `doctor` still report it, because headroom's own capture cadence (often sub-minute, against usage quantized to whole percentage points) saturates that measurement on essentially any real segment, independent of whether the underlying usage is actually bursty -- it is structurally uninformative for input shaped like headroom's own. This policy exists precisely because a hard cutoff cannot live inside the projection itself; see [Why the bounds are sound](explanation-bounds.md) for why that separation matters and what the policy declines to claim. `status` prints one line per window whose projection clears both checks; a declined, untrustworthy, or stale-evidence projection is silent there, matching `hook`'s existing behavior of saying nothing when there is nothing worth saying.
